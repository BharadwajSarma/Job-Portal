...
const Applyjobs = () => {
  const { id } = useParams(); // Corrected extraction of 'id'
  const [JobData, setJobData] = useState(null);
  const { jobs } = useContext(AppContext);

  const fetchJob = async () => {
    const data = jobs.filter(job => job._id === id); // Use id directly
    if (data.length !== 0) {
      setJobData(data[0]); // Set the first matching job
      console.log(data[0]);
    }
  };
        ...
        <div className='bg-white text-black rounded-lg w-ful'>
          <div className='flex justify-center md:justify-between flex-wrap gap-8 px-14 py-20 mb-6 bg-sky-50 border-sky-400 rounded-xl '>
            ...
            <div className='flex flex-col justify-center text-end text-sm max-md:mx-auto max-md:text-center'>
              ...
              <p className='mt-1 text-gray-600'>Posted {moment(JobData.date).fromNow()}</p> {/* Correct property */}
            </div>
          </div>
